...
export class EmployeeController {
  ...
  @get('/employees/getAvgSalaryByDepartment/{id}')
  @response(200, {
    description: 'Employee model instance',
    content: {
      'application/json': {
  ...
  async getAvgSalaryByDepartment(
    @param.path.string('id') department: string
  ) {
    let employees = this.employeeRepository.find({where: {department: department}});
    ...
    return initialSalary / (await employees).length;
  }
